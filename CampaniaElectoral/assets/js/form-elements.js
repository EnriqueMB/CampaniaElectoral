var FormElements = function() {"use strict";
	...
	var runCalbackIcheck = function () {
	    $('input.checkbox-callback').on('ifChecked', function (event) {
	        //$("chkTodos").val = true;
	        $("#chkTodos").attr("value", true);
	        $(".js-example-disabled-multi").prop("disabled", true);
	        $("#frmMaster").find('select').prop('selectedIndex', -1);
	        $("#cmbColaborador").trigger('change.select2');
	    });
	    $('input.checkbox-callback').on('ifUnchecked', function (event) {
	        $("#chkTodos").attr("value", false);
	...
	var runCKEditor = function () {
	    var nodoTr = document.getElementsByClassName("ckeditor");
	    if (nodoTr.length > 0) {
